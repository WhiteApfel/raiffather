...
class RaiffatherTransactions(RaiffatherBase):
    async def transactions(
        ...
        """
        Получить транзакции, можно только последние три месяца пока что
        это ограничения со стороны Райфа

        :param size: всегда 25, лучше не менять, райф не умеет с этим работать,
            хотя зачем-то заявляет
    ...
    async def global_history_generator(self) -> AsyncGenerator[Transaction, None]:
        """
        Генератор для итерации страниц из истории операций, но дальше трёх
        страниц уйти получить не получится
        Но оно само остановится, когда увидит, что пошло по старой.
        Может быть полезно, да. И удобно. Благодарите.
